...
import type { AuthResponse, StorefrontRegisterRequest } from "@/types/api";

export async function POST(request: NextRequest) {
  const body = await safeJsonBody<StorefrontRegisterRequest>(request);
  if (!body) return badRequest();

  try {
    const auth = await apiFetch<AuthResponse>(`/api/shop/${BUSINESS_SLUG}/auth/register`, {
      method: "POST",
      body,
    });
